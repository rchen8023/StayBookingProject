...
class RemoveStayButton extends React.Component {
  ...
  handleRemoveStay = async () => {
    const { stay, onRemoveSucess } = this.props;
...
class MyStays extends React.Component {
  ...
  //What is await and how to use it?

  // line1: const a = await func();
  // line2: console.log(a);
  // what are line 1 and line 2 doing?
  //func().then(a => consol.log(a));
  ...
  loadData = async () => {
    ...
    try {
      const resp = await getStaysByHost(); // want to use await, the function must with a keyword async
      this.setState({
        data: resp,
      });
    } catch (error) {
  ...
  render() {
    return (
      <List
        ...
        dataSource={this.state.data} //[{}, {}, {}] --> data: array of object
        renderItem={(item) => (
          <List.Item>
            <Card
              key={item.id}
              ...
              extra={<RemoveStayButton stay={item} onRemoveSucess={this.loadData}/>}
              // style={{ width: 400 }}
            >
              {
                <Carousel
                ...
                >
                  {item.images.map((image, index) => (
                    <div key={index}>
                      <Image src={image.url} width="100%" height={350}/>
                    </div>
                  ))}
                </Carousel>
              }
